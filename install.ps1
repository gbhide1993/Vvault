...
# Kill any native Ollama process that may be holding port 11434
...
while ($waited -lt $maxWait) {
    ...
    $waited += 5
    try {
        $response = Invoke-WebRequest -Uri "https://localhost:3443" -UseBasicParsing -TimeoutSec 3 -SkipCertificateCheck -ErrorAction SilentlyContinue
...
if ($ready) {
    ...
    Write-Host ""
    Write-Host "  Default login: admin / changeme123" -ForegroundColor Yellow
